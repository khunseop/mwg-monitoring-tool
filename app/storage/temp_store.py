...
def get_latest_batch_path(proxy_id: int) -> Optional[str]:
    """
    Return the path to the latest batch file for a proxy, or None if not found.
    """
    batches = list_batches(proxy_id)
    if not batches:
        return None
    latest_path = batches[0][1]
...
def read_latest(proxy_id: int) -> List[Dict[str, Any]]:
    ...
    """
    path = get_latest_batch_path(proxy_id)
    if not path:
        return []
